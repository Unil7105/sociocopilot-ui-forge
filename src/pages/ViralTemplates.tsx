  ...
  return (
    <TooltipProvider>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-purple-50/20">
        ...
        <div className="bg-gradient-to-r from-purple-500 via-indigo-500 to-blue-500 pt-16 pb-20 px-8">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="max-w-2xl mx-auto mb-8">
              <div className="relative">
                ...
                <Input
                  placeholder="Search viral templates..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="pl-14 h-14 bg-white/95 backdrop-blur-sm shadow-2xl border-0 focus:ring-4 focus:ring-white/30 hover:shadow-3xl transition-all duration-300 text-lg rounded-2xl font-medium placeholder:text-gray-500"
                />
              </div>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {categories.map((category) => (
                <Button
                  ...
                  className={`px-6 py-3 font-semibold transition-all duration-300 rounded-full text-sm hover:scale-105 ${
                    selectedCategory === category.name 
                      ? "bg-white text-purple-600 shadow-xl scale-105 hover:shadow-2xl" 
        ...
        <div className="px-8 py-16 max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            {filteredTemplates.map((template) => (
              <Card 
                key={template.id} 
                className="group hover:shadow-2xl hover:scale-[1.02] transition-all duration-300 ease-out border-0 shadow-lg bg-white backdrop-blur-md rounded-3xl overflow-hidden relative will-change-transform"
              >
                {/* Trending/New Badges */}
                {template.trending && (
                  <div className="absolute top-4 right-4 z-10">
                    <Badge className="bg-gradient-to-r from-orange-400 to-red-500 text-white border-0 px-3 py-1 text-xs font-bold rounded-full shadow-lg">
                      🔥 Most Used
                    </Badge>
                  </div>
                )}
                ...
                <CardContent className="p-0 h-full flex flex-col">
                  ...
                  <div className="p-8 flex-1 flex flex-col">
                    ...
                    <div className="flex gap-4 pt-6 border-t border-gray-100 mt-auto">
                      <Button 
                        size="sm" 
                        className="flex-1 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 shadow-lg hover:shadow-xl transition-all duration-300 font-semibold h-12 rounded-xl relative overflow-hidden group/btn"
                      >
                        ...
                        <TooltipTrigger asChild>
                          ...
                          >
                            <Heart className={`w-5 h-5 transition-all duration-300 ${
                              savedTemplates.includes(template.id) ? "fill-current scale-110" : ""
                            }`} />
                          </Button>
                        </TooltipTrigger>
                        <TooltipContent>
          ...
          <div className="flex justify-center mt-16">
            <Button 
              size="lg"
              className="bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 shadow-xl hover:shadow-2xl transition-all duration-300 px-8 py-4 text-lg font-semibold rounded-2xl"
            >
